...
private:
    bool parseLine(const std::string& line, WeatherSample& sample) {
        ...
        // Parse: day,hour,temperature,humidity
...
// ============================================================
// MAIN: Part 2 - SHARED FILE (I/O Bottleneck Demonstration)
// ============================================================
int main(int argc, char** argv) {
    ...
    int rank, size;
    // MPI_Comm_rank: Get my unique process ID (rank)
    ...
    if (rank == 0) {
        ...
        std::cout << "=== Part 2: SHARED FILE + Send/Receive ===\n\n";
        std::cout << "Number of MPI processes: " << size << "\n\n";
    }

    // ========================================
    // DATA LOADING: All ranks read from SHARED CSV file
    // ========================================
    ...
    std::size_t localRows = endRow - startRow;

    if (rank == 0) {
        std::cout << "=== Loading from Shared CSV File ===\n";
        std::cout << "File: weather_data_full.csv (500 MB, 10M rows)\n";
        std::cout << "Each rank reads its row range [startRow, endRow)\n\n";
    }

    for (int r = 0; r < size; ++r) {
        if (rank == r) {
            std::cout << "Rank " << r << " will read rows [" << startRow 
                      << " - " << (endRow - 1) << "] from shared file\n";
        }
    ...
    // Load data from shared CSV
    std::vector<WeatherSample> localData;
    CSVReader reader;

    auto loadStart = std::chrono::high_resolution_clock::now();
    std::size_t localSize = reader.loadFromCSV("weather_data_full.csv", localData, startRow, endRow);
    ...
    for (int r = 0; r < size; ++r) {
        if (rank == r) {
            if (localSize == 0) {
                std::cout << "[t=" << loadAbsoluteMs << "ms] Rank " << rank << ": ERROR - file not found\n";
            } else {
                std::cout << "[t=" << loadAbsoluteMs << "ms] Rank " << rank << " loaded " << localSize 
                          << " samples, local_total = " << localTotal 
                          << " (load: " << loadMs << " ms, compute: " << computeMs << " ms)\n";
            }
        }
        // MPI_Barrier: Synchronization point before communication
        MPI_Barrier(MPI_COMM_WORLD);
    }

    if (rank == 0) {
        std::cout << "\n=== Communication Phase: Send/Receive (O(N) sequential) ===\n\n";
    }

    // ========================================
    // COMMUNICATION: Point-to-Point Send/Recv (O(N))
    // ========================================
    double globalTotal = 0.0;
    
    if (rank == 0) {
        // ROOT PROCESS: Sequential receives
        ...
        // WORKER PROCESSES: Send to rank 0
        // MPI_Send: BLOCKING send - waits until rank 0 receives
